...
app.layout = html.Div([
    html.Div([
        ### left side
        html.H1('Factors extreme points - Coherence'),
        dcc.Markdown("""
...
@app.callback(
    ...
    Input('rolling-or-not', 'value')
    )
def update_pairs_graph(pair, factors, line, q, rolling_or_not):
    
    if rolling_or_not == 'intrady':
        WINDOW = 1
    else:
        WINDOW = WINDOW_SIZE
    
    
    oil_data = get_oil_data('BZ=F', START, END, INTERVAL, WINDOW, q)
